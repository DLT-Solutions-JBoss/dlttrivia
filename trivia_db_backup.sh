...
mysqldump trivia user question choice question_choice tag question_tag contest contestant scheduled_question ask response hibernate_sequence --user=$TRIVIA_DB_USER --password=$TRIVIA_DB_PWD --host=127.0.0.1 --port=$TRIVIA_DB_LOCAL_PORT --no-create-info --skip-triggers --compact > ./src/main/resources/init_data_2.sql
